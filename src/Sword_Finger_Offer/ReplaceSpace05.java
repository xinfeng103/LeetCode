...
// todo 5. 实现一个函数，把字符数组arr中的每个空格替换成"%20"。例如，输入：s = "We are happy."  输出："We%20are%20happy."

/* todo 分析：正向替换会导致空格后面的字符多次被移动，复杂度为O(n^2)
             反向替换：先遍历一遍字符串，统计出字符串中空格的总数，替换后字符串的总长度为 原来的长度+2*空格数，复杂度为O(n)
